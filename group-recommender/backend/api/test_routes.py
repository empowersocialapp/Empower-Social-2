...
from backend.recommendation.learning import AdaptiveLearningSystem
...
@router.get("/learning-metrics/{user_id}")
def get_learning_metrics(user_id: str):
    """Get adaptive learning metrics for a user"""
    import json
    adaptive_learning = AdaptiveLearningSystem()
    result = adaptive_learning.get_learned_weights_with_confidence(user_id)
    
    # Get feedback summary for additional context
    feedback_summary = adaptive_learning.get_feedback_summary(user_id)
    
    # Calculate weight changes from defaults
    default_weights = adaptive_learning.DEFAULT_WEIGHTS
    weight_changes = {
        key: result['weights'][key] - default_weights[key]
    ...
    return {
        ...
        "feedback_summary": {
            "total_feedback": feedback_summary['total_feedback'],
            "with_reasons": feedback_summary['with_reasons'],
            "reason_counts": feedback_summary['reason_counts']
        },
        "interpretation": {
            "confidence_level": "high" if result['confidence'] > 0.7 else "moderate" if result['confidence'] > 0.4 else "low",
            "consistency_level": "high" if result['consistency_score'] > 0.6 else "moderate" if result['consistency_score'] > 0.3 else "low"
        }
    }
